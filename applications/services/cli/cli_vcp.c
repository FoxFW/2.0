...
typedef struct {
    enum {
        ...
        CliVcpMessageTypeSessionLock,
        CliVcpMessageTypeSessionUnlock,
    } type;
...
struct CliVcp {
    ...
    bool is_enabled, is_connected, session_locked;
...
static void cli_vcp_message_received(FuriEventLoopObject* object, void* context) {
    ...
    case CliVcpMessageTypeSessionLock:
        if(cli_vcp->session_locked) break;
        FURI_LOG_D(TAG, "Session lock");
        cli_vcp->session_locked = true;
        // Close active shell without touching USB hardware
        if(cli_vcp->is_connected) {
            cli_vcp_signal_internal_event(cli_vcp, CliVcpInternalEventDisconnected);
        }
        break;

    case CliVcpMessageTypeSessionUnlock:
        if(!cli_vcp->session_locked) break;
        FURI_LOG_D(TAG, "Session unlock");
        cli_vcp->session_locked = false;
        // If qFlipper's DTR is still asserted, start a fresh session immediately
        if(cli_vcp->is_enabled &&
           (furi_hal_cdc_get_ctrl_line_state(VCP_IF_NUM) & CdcCtrlLineDTR)) {
            cli_vcp_signal_internal_event(cli_vcp, CliVcpInternalEventConnected);
        }
        break;
    }
...
static void cli_vcp_internal_event_happened(FuriEventLoopObject* object, void* context) {
    ...
    case CliVcpInternalEventConnected: {
        if(cli_vcp->is_connected) return;
        if(cli_vcp->session_locked) return; // USB stays up but sessions are blocked
...
static CliVcp* cli_vcp_alloc(void) {
    ...
    // Explicit init — malloc does not zero memory, warm reboots leave garbage
    cli_vcp->is_enabled              = true;
    cli_vcp->is_connected            = false;
    cli_vcp->session_locked          = false;
    cli_vcp->own_pipe                = NULL;
    ...
    // Register CDC callbacks now — event loop not running yet so do it directly
    cli_vcp->previous_interface = furi_hal_usb_get_config();
    furi_hal_usb_set_config(&usb_cdc_single, NULL);
    furi_hal_cdc_set_callbacks(VCP_IF_NUM, &cdc_callbacks, cli_vcp);
...
void cli_vcp_session_lock(CliVcp* cli_vcp) {
    furi_check(cli_vcp);
    CliVcpMessage message = {
        .type = CliVcpMessageTypeSessionLock,
    };
    cli_vcp_synchronous_request(cli_vcp, &message);
}
...
void cli_vcp_session_unlock(CliVcp* cli_vcp) {
    furi_check(cli_vcp);
    CliVcpMessage message = {
        .type = CliVcpMessageTypeSessionUnlock,
    };
    cli_vcp_synchronous_request(cli_vcp, &message);
}
