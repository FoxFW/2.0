...
static void fox_splash_draw_cb(Canvas* canvas, void* model) {
    ...
    if(splash == NULL || canvas == NULL || splash->icon == NULL) return;
...
static void fox_splash_timer_cb(void* context) {
    ...
    if(splash == NULL || splash->finished) return;
    ...
    if(splash->elapsed_ms >= total_ms) {
        ...
        if(splash->timer != NULL) furi_timer_stop(splash->timer);
        if(splash->view != NULL) {
...
void fox_splash_free(FoxSplash* splash) {
    if(splash == NULL) return;
    if(splash->timer != NULL) furi_timer_free(splash->timer);
    if(splash->view != NULL) view_free(splash->view);
...
void fox_splash_start(FoxSplash* splash) {
    if(splash == NULL || splash->view == NULL || splash->timer == NULL) return;
