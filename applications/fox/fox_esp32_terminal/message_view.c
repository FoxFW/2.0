...
static bool message_input_cb(InputEvent* event, void* context) {
    ...
    if(app->message_view_serial_retry_failed) {
        ...
        case InputKeyOk:
            if(app->message_view_not_detected_focus_left) {
                if(app->view_dispatcher != NULL) view_dispatcher_stop(app->view_dispatcher);
                return true;
            }
            ...
            if(app->serial_retry_timer != NULL) furi_timer_start(app->serial_retry_timer, 500);
            return true;
        default:
            return false;
        }
    ...
    case InputKeyOk:
        if(app->message_view_not_detected_focus_left) {
            ...
            app->current_view = FoxTerminalViewConnectSettings;
            if(app->view_dispatcher != NULL) {
                view_dispatcher_switch_to_view(app->view_dispatcher, FoxTerminalViewConnectSettings);
            }
        } else {
