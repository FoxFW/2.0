...
static void esp_at_rx_callback(FuriHalSerialHandle* handle, FuriHalSerialRxEvent event, void* context) {
    ...
    if(esp_at == NULL) return;
    if(event == FuriHalSerialRxEventData) {
        uint8_t byte = furi_hal_serial_async_rx(handle);
        if(esp_at->rx_stream != NULL) {
            furi_stream_buffer_send(esp_at->rx_stream, &byte, 1, 0);
        }
        if(esp_at->raw_capture_enabled && esp_at->raw_capture_stream != NULL) {
...
static void esp_at_emit_line(EspAt* esp_at, const char* text, size_t length) {
    if(esp_at == NULL || esp_at->msg_queue == NULL) return;
    if(length == 0) return;
    EspAtMsg msg;
...
EspAt* esp_at_alloc(FuriHalSerialId serial_id, uint32_t baud_rate) {
    ...
    if(esp_at->rx_stream == NULL || esp_at->raw_capture_stream == NULL ||
       esp_at->msg_queue == NULL) {
    ...
    if(esp_at->worker == NULL) {
        furi_hal_serial_async_rx_stop(esp_at->serial);
        if(esp_at->serial_owned) furi_hal_serial_deinit(esp_at->serial);
...
void esp_at_raw_capture_start(EspAt* esp_at) {
    if(esp_at == NULL || esp_at->raw_capture_stream == NULL) return;
    uint8_t discard;
...
size_t esp_at_raw_capture_read(EspAt* esp_at, uint8_t* out, size_t out_capacity, uint32_t timeout_ms) {
    if(esp_at == NULL || esp_at->raw_capture_stream == NULL || out == NULL || out_capacity == 0) {
        return 0;
    }
...
void esp_at_send(EspAt* esp_at, const char* command) {
    if(esp_at == NULL || esp_at->serial == NULL || command == NULL) return;
...
bool esp_at_receive(EspAt* esp_at, EspAtMsg* msg, uint32_t timeout_ms) {
    if(esp_at == NULL || esp_at->msg_queue == NULL || msg == NULL) return false;
