...
static void message_draw_cb(Canvas* canvas, void* model) {
    ...
    if(app->message_view_not_supported) {
        canvas_set_font(canvas, FontPrimary);
        canvas_draw_str_aligned(canvas, 64, 14, AlignCenter, AlignCenter, "FoxLAB Unsupported");
        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str_aligned(canvas, 64, 30, AlignCenter, AlignCenter, "Not enough RAM on");
        canvas_draw_str_aligned(canvas, 64, 40, AlignCenter, AlignCenter, "this ESP32 (S2).");

        message_draw_one_button(canvas, "Back");
        return;
    }
...
static bool message_input_cb(InputEvent* event, void* context) {
    ...
    case InputKeyOk:
        if(app->message_view_not_detected_focus_left) {
            ...
            app->current_view = FoxLabViewConnectSettings;
            view_dispatcher_switch_to_view(app->view_dispatcher, FoxLabViewConnectSettings);
        } else {
...
void message_view_show_detecting(App* app) {
    ...
    app->message_view_not_supported = false;
...
void message_view_show_not_supported(App* app) {
    app->message_view_detecting           = false;
    app->message_view_not_detected_focus_left = false;
    app->message_view_serial_busy         = false;
    app->message_view_serial_retrying     = false;
    app->message_view_serial_retry_failed = false;
    app->message_view_not_supported       = true;
    app->current_view = FoxLabViewMessage;
    view_dispatcher_switch_to_view(app->view_dispatcher, FoxLabViewMessage);
}
