...
#define FOX_ESP32_COMMANDER_GPIO_REMAP_FILE_NAME ".fox_esp32_commander_gpio_remap.settings"
#define FOX_ESP32_COMMANDER_GPIO_REMAP_PATH INT_PATH(FOX_ESP32_COMMANDER_GPIO_REMAP_FILE_NAME)
#define FOX_ESP32_COMMANDER_GPIO_REMAP_VER  (1)
#define FOX_ESP32_COMMANDER_GPIO_REMAP_MAGIC (0x1A)

static inline void gpio_remap_settings_save(const GpioRemapSettings* settings) {
    furi_assert(settings);
    saved_struct_save(
        FOX_ESP32_COMMANDER_GPIO_REMAP_PATH,
        settings,
        sizeof(GpioRemapSettings),
        FOX_ESP32_COMMANDER_GPIO_REMAP_MAGIC,
        FOX_ESP32_COMMANDER_GPIO_REMAP_VER);
}
