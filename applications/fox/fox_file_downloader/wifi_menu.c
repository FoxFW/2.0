...
void wifi_network_list_show(App* app, bool for_connect) {
    ...
    view_dispatcher_switch_to_view(app->view_dispatcher, FoxDownloaderViewNetworkList);
}

#define FOX_WIFI_STATUS_PATH "/ext/apps_data/fox_esp32/wifi_status.txt"
...
static void fox_wifi_status_write(bool connected) {
    ...
    if(storage_file_open(file, FOX_WIFI_STATUS_PATH, FSAM_WRITE, FSOM_CREATE_ALWAYS)) {
...
bool fox_wifi_status_read(void) {
    Storage* storage = furi_record_open(RECORD_STORAGE);
    File* file = storage_file_alloc(storage);
    bool connected = false;
    if(storage_file_open(file, FOX_WIFI_STATUS_PATH, FSAM_READ, FSOM_OPEN_EXISTING)) {
        char buf[1] = {0};
        if(storage_file_read(file, buf, 1) == 1) connected = (buf[0] == '1');
    }
    storage_file_close(file);
    storage_file_free(file);
    furi_record_close(RECORD_STORAGE);
...
static void station_list_show(App* app) {
    ...
    view_dispatcher_switch_to_view(app->view_dispatcher, FoxDownloaderViewStationList);
}
