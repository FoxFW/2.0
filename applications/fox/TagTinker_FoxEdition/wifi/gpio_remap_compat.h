...
#define TAGTINKER_FOXEDITION_GPIO_REMAP_FILE_NAME ".tagtinker_foxedition_gpio_remap.settings"
#define TAGTINKER_FOXEDITION_GPIO_REMAP_PATH INT_PATH(TAGTINKER_FOXEDITION_GPIO_REMAP_FILE_NAME)
#define TAGTINKER_FOXEDITION_GPIO_REMAP_VER  (1)
#define TAGTINKER_FOXEDITION_GPIO_REMAP_MAGIC (0x1A)

static inline void gpio_remap_settings_save(const GpioRemapSettings* settings) {
    furi_assert(settings);
    saved_struct_save(
        TAGTINKER_FOXEDITION_GPIO_REMAP_PATH,
        settings,
        sizeof(GpioRemapSettings),
        TAGTINKER_FOXEDITION_GPIO_REMAP_MAGIC,
        TAGTINKER_FOXEDITION_GPIO_REMAP_VER);
}
