...
#define CONNECT_BOX_X  4
#define CONNECT_BOX_W  120
#define CONNECT_BOX_H  22
#define CONNECT_BOX_R  4

static const uint8_t k_connect_box_y[ConnectSettingsRowCount] = {13, 38};
...
static void connect_settings_draw_cb(Canvas* canvas, void* model) {
    ...
    for(size_t row = 0; row < ConnectSettingsRowCount; row++) {
        bool selected   = (row == (size_t)app->connect_settings_selected);
        bool adjustable = (row == ConnectSettingsRowPins);
        uint8_t y = k_connect_box_y[row];

        char row_text[32];
        switch((ConnectSettingsRow)row) {
        case ConnectSettingsRowPins:
            snprintf(row_text, sizeof(row_text), "Pins: %s",
                     app_pin_option_label(app->pin_option_index));
            break;
        ...
        if(selected) {
            canvas_draw_rbox(canvas, CONNECT_BOX_X, y, CONNECT_BOX_W, CONNECT_BOX_H, CONNECT_BOX_R);
            canvas_set_color(canvas, ColorWhite);
        } else {
            /* Double border — outer frame + 2px inset inner frame for the Fox button look */
            canvas_draw_rframe(canvas, CONNECT_BOX_X, y, CONNECT_BOX_W, CONNECT_BOX_H, CONNECT_BOX_R);
            canvas_draw_rframe(canvas, CONNECT_BOX_X + 2, y + 2,
                               CONNECT_BOX_W - 4, CONNECT_BOX_H - 4, CONNECT_BOX_R - 2);
        }

        uint8_t text_y = y + CONNECT_BOX_H / 2;
        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str_aligned(canvas, 64, text_y, AlignCenter, AlignCenter, row_text);

        if(adjustable) {
            /* Arrows inside the box — inherit current color (white-on-black or black-on-white)
               so they're always visible regardless of selection state. */
            canvas_draw_str_aligned(canvas, CONNECT_BOX_X + 6, text_y, AlignLeft,  AlignCenter, "<");
            canvas_draw_str_aligned(canvas, CONNECT_BOX_X + CONNECT_BOX_W - 6, text_y, AlignRight, AlignCenter, ">");
        }
