...
static bool navigation_callback(void* context) {
    ...
    if(app->current_view == FoxCommanderViewMenu) {
        if(app->menu_context == MenuContextFoxPortal) {
    ...
    if(app->current_view == FoxCommanderViewFoxPortalQr) {
        app_switch_to_menu(app, MenuContextFoxPortal);
        return true;
    }
...
static App* app_alloc(void) {
    ...
    app->foxportal_qr_view = foxportal_qr_view_alloc(app);
    ...
    view_dispatcher_add_view(
        app->view_dispatcher, FoxCommanderViewFoxPortalQr, app->foxportal_qr_view);
...
static void app_free(App* app) {
    ...
    view_dispatcher_remove_view(app->view_dispatcher, FoxCommanderViewFoxPortalQr);
    ...
    view_free(app->foxportal_qr_view);
