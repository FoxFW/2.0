...
void update_meta_find(Storage* storage, UpdaterFlow flow, DownloadedMeta* out) {
    ...
    if(storage_dir_open(dir, UPDATER_DOWNLOAD_DIR)) {
        while(storage_dir_read(dir, &info, name, sizeof(name))) {
            ...
            snprintf(meta_name, sizeof(meta_name), "%s", name);
        }
    }
    storage_dir_close(dir);
    storage_file_free(dir);

    if(meta_name[0] == '\0') return;

    char meta_path[UPDATER_PATH_LEN];
    snprintf(meta_path, sizeof(meta_path), "%s/%s", UPDATER_DOWNLOAD_DIR, meta_name);
    ...
    if(flow != UpdaterFlowNone && found_flow != flow) return;

    char asset_path[UPDATER_PATH_LEN] = {0};
    json_mini_get_string(buf, "asset", asset_path, sizeof(asset_path));
    if(asset_path[0] == '\0') return;

    FileInfo stat_info;
    if(storage_common_stat(storage, asset_path, &stat_info) != FSE_OK) return;
    ...
    if(stat_info.flags & FSF_DIRECTORY) {
        ...
        for(uint8_t k = 0; k < 3; k++) {
            ...
            FileInfo sub_info;
            if(storage_common_stat(storage, sub_path, &sub_info) != FSE_OK) return;
            total += (uint32_t)sub_info.size;
        }
        if(total != size) return;
    } else {
        if((uint32_t)stat_info.size != size) return;
    }
    ...
    json_mini_get_string(buf, "board", out->board_folder, sizeof(out->board_folder));
    out->size = size;
}
