...
static void board_draw(Canvas* canvas, void* model_ptr) {
    ...
    for(uint8_t row = 0; row < 2; row++) {
        ...
        if(row == 1) {
            if(sel) {
                updater_draw_ok_button(canvas, BOX_X, y, BOX_W, BOX_H, BOX_R, "Continue");
                continue;
            }
            canvas_set_color(canvas, ColorBlack);
            canvas_draw_rframe(canvas, BOX_X, y, BOX_W, BOX_H, BOX_R);
            canvas_set_font(canvas, FontSecondary);
            canvas_draw_str_aligned(canvas, 64, y + BOX_H / 2, AlignCenter, AlignCenter, "Continue");
            continue;
        }
        ...
        canvas_draw_str_aligned(
            canvas, 64, ty, AlignCenter, AlignCenter, k_updater_boards[app->board_index].label);
...
static bool board_input(InputEvent* event, void* context) {
    ...
    case InputKeyUp:
    case InputKeyDown:
        with_view_model(app->board_view, BoardModel * m, { m->row = m->row == 0 ? 1 : 0; }, true);
        return true;
    case InputKeyLeft:
    case InputKeyRight: {
        uint8_t row = 0;
        ...
        if(row == 0) {
            if(event->key == InputKeyRight) {
                app->board_index = (app->board_index + 1) % UPDATER_BOARD_COUNT;
            } else {
                app->board_index =
                    (app->board_index == 0) ? UPDATER_BOARD_COUNT - 1 : app->board_index - 1;
            }
    ...
    case InputKeyOk:
        view_dispatcher_send_custom_event(app->view_dispatcher, UpdaterEventBoardGo);
        return true;
    default:
        return false;
    }
