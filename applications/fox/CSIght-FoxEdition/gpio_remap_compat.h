...
#define CSIGHT_FOXEDITION_GPIO_REMAP_FILE_NAME ".csight_foxedition_gpio_remap.settings"
#define CSIGHT_FOXEDITION_GPIO_REMAP_PATH INT_PATH(CSIGHT_FOXEDITION_GPIO_REMAP_FILE_NAME)
#define CSIGHT_FOXEDITION_GPIO_REMAP_VER  (1)
#define CSIGHT_FOXEDITION_GPIO_REMAP_MAGIC (0x1A)

static inline void gpio_remap_settings_save(const GpioRemapSettings* settings) {
    furi_assert(settings);
    saved_struct_save(
        CSIGHT_FOXEDITION_GPIO_REMAP_PATH,
        settings,
        sizeof(GpioRemapSettings),
        CSIGHT_FOXEDITION_GPIO_REMAP_MAGIC,
        CSIGHT_FOXEDITION_GPIO_REMAP_VER);
}
