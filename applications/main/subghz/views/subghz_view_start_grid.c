...
/* ── Icon declarations (generated by FBT from images/ folder) ────────── */
...
/* Icon for the Garage Door Remote grid button — generated by FBT from
 * assets/icons/Sub-GHz/gdr_10x10.png (same category as the other grid icons) */
extern const Icon I_gdr_10x10;
/* Icon for the RF Jammer grid button — generated by FBT from
 * assets/icons/Sub-GHz/rf_jammer_10x10.png */
extern const Icon I_rf_jammer_10x10;

/* ── Screen layout ─────────────────────────────────────────────────────
 * Two big rows visible at a time, Fox-style rounded buttons.
 *
 * y=0..3   : up-scroll indicator (triangle when rows above)
 * y=4..29  : top visible button row  (BTN_H = 26)
 * y=30..33 : inter-row gap (4px)
 * y=34..59 : bottom visible button row
 * y=60..63 : down-scroll indicator
 *
 * Button interior (26px tall):
 *   y+3 .. y+12 : 10x10 icon (centered horizontally)
 *   y+13..y+14  : 2px gap
 *   y+15..y+22  : label (FontSecondary ~7px, centred in remaining space)
 * ────────────────────────────────────────────────────────────────────── */
#define BTN_H        26
#define BTN_R         5
#define ROW_TOP_Y     4
...
/* ── Navigation: skip invisible buttons ──────────────────────────────── */
...
static void sgrid_draw_cb(Canvas* canvas, void* _model) {
    ...
    uint8_t wr = m->window_row;

    /* Max visible row (for scroll indicator) */
    uint8_t max_row = 0;
    for(uint8_t i = 0; i < SGRID_BTN_COUNT; i++)
        if(m->visible[i] && k_btns[i].row > max_row)
            max_row = k_btns[i].row;
    ...
    for(uint8_t i = 0; i < SGRID_BTN_COUNT; i++) {
        ...
        uint8_t screen_y;
        if(r == wr)        screen_y = ROW_TOP_Y;
        else if(r == wr+1) screen_y = ROW_BOT_Y;
        else               continue;
        draw_btn(canvas, i, screen_y, i == m->selected);
    }

    /* Scroll arrows — hidden at the boundary they indicate.
     * Navigation still wraps at both ends; the arrows just reflect whether
     * there is visually more content in that direction. */
    canvas_set_color(canvas, ColorBlack);
    if(wr > 0)                  draw_arrow_up(canvas);   /* hide at top    */
    if(wr + 1 < max_row)        draw_arrow_down(canvas); /* hide at bottom */
}
