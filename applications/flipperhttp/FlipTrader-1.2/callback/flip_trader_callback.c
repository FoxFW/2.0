...
// ==============================================================================
// FIRMWARE COMPATIBILITY FIX: Added `AssetLoaderModel *model` parameter.
// 
// WHY: The function flip_trader_generic_switch_to_view() expects the 'fetcher' 
// argument to match the typedef 'AssetLoaderFetch'.
// 'AssetLoaderFetch' is defined as returning a bool and taking (AssetLoaderModel *model).
// By updating the signature here, we resolve the "incompatible pointer type" 
// compile error on newer/custom firmware builds.
// ==============================================================================
static bool send_price_request(AssetLoaderModel *model)
{
    // We use the UNUSED macro because the 'model' parameter is required by the 
    // typedef signature, but we do not actually need to use it inside this function.
    // This prevents the compiler from throwing an "unused variable" warning.
    UNUSED(model); 
...
// FIRMWARE COMPATIBILITY FIX: Added `AssetLoaderModel *model` parameter.
// 
// WHY: Similar to the fetcher function above, flip_trader_generic_switch_to_view() 
// expects the 'parser' argument to match the typedef 'AssetLoaderParser'.
// 'AssetLoaderParser' is defined as returning a char* and taking (AssetLoaderModel *model).
// This change aligns the function signature to fix the second compile error.
// ==============================================================================
static char *process_asset_price(AssetLoaderModel *model)
{
    // Again, we satisfy the compiler's strictness about unused variables.
    UNUSED(model);
...
{
    // ==============================================================================
    // FIRMWARE COMPATIBILITY FIX: The function call itself remains unchanged!
    // Now that send_price_request and process_asset_price have the correct 
    // signatures (accepting AssetLoaderModel *), they will cleanly cast to 
    // AssetLoaderFetch and AssetLoaderParser without throwing -Werror pointer types.
    // ==============================================================================
