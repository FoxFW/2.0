...
#include "subghz_wardriving_scene.h"
...
static bool subghz_scene_receiver_info_update_parser(void* context) {
    ...
    if(subghz_wardriving_txrx_load_decoder_by_name_protocol(
           subghz->txrx,
           subghz_wardriving_history_get_protocol_name(subghz->history, subghz->idx_menu_chosen))) {
        // we are trying to deserialize without checking for errors, since it is assumed that we just received this chignal
        subghz_protocol_decoder_base_deserialize(
            subghz_wardriving_txrx_get_decoder(subghz->txrx),
            subghz_wardriving_history_get_raw_data(subghz->history, subghz->idx_menu_chosen));

        SubGhzRadioPreset* preset =
            subghz_wardriving_history_get_radio_preset(subghz->history, subghz->idx_menu_chosen);

        //Edit TX power, if necessary.
        subghz_wardriving_txrx_set_tx_power(preset->data, preset->data_size, subghz->tx_power);

        subghz_wardriving_txrx_set_preset(
            subghz->txrx,
...
void subghz_scene_receiver_info_draw_widget(SubGhz* subghz) {
    if(subghz_scene_receiver_info_update_parser(subghz)) {
        ...
        subghz_wardriving_txrx_get_frequency_and_modulation(
        ...
        subghz_protocol_decoder_base_get_string(
            subghz_wardriving_txrx_get_decoder(subghz->txrx), text);
        ...
        if(subghz_wardriving_txrx_protocol_is_serializable(subghz->txrx)) {
            widget_add_button_element(
                subghz->widget,
                GuiButtonTypeRight,
                "Save",
        ...
        if(subghz_wardriving_txrx_protocol_is_transmittable(subghz->txrx, false)) {
            widget_add_button_element(
                subghz->widget,
                GuiButtonTypeCenter,
                "Send",
...
void subghz_scene_receiver_info_on_enter(void* context) {
    ...
    if(!subghz_wardriving_history_full(subghz->history)) {
...
bool subghz_scene_receiver_info_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        if(event.event == SubGhzCustomEventSceneReceiverInfoTxStart) {
            ...
            subghz_wardriving_txrx_hopper_pause(subghz->txrx);
            ...
            if(!subghz_tx_start(
                   subghz,
                   subghz_wardriving_history_get_raw_data(
                       subghz->history, subghz->idx_menu_chosen))) {
                subghz_wardriving_txrx_rx_start(subghz->txrx);
                subghz_wardriving_txrx_hopper_unpause(subghz->txrx);
                subghz->state_notifications = SubGhzNotificationStateRx;
            ...
            subghz_wardriving_txrx_hopper_set_state(subghz->txrx, SubGhzHopperStateOFF);
            ...
            if(subghz_wardriving_txrx_protocol_is_serializable(subghz->txrx)) {
                ...
                subghz->save_datetime = subghz_wardriving_history_get_datetime(
                    subghz->history, subghz->idx_menu_chosen);
        ...
        if(subghz_wardriving_txrx_hopper_get_state(subghz->txrx) != SubGhzHopperStateOFF) {
            subghz_wardriving_txrx_hopper_update(
                subghz->txrx, subghz->last_settings->hopping_threshold);
        }
...
void subghz_scene_receiver_info_on_exit(void* context) {
    ...
    widget_reset(subghz->widget);
    subghz_wardriving_txrx_reset_dynamic_and_custom_btns(subghz->txrx);
}
