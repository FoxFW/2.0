...
bool subghz_tx_start(SubGhz* subghz, FlipperFormat* flipper_format) {
    switch(subghz_wardriving_txrx_tx_start(subghz->txrx, flipper_format)) {
...
bool subghz_key_load(SubGhz* subghz, const char* file_path, bool show_dialog) {
    ...
    Stream* fff_data_stream =
        flipper_format_get_raw_stream(subghz_wardriving_txrx_get_fff_data(subghz->txrx));
    ...
    do {
        ...
        if(!subghz_wardriving_txrx_radio_device_is_frequency_valid(subghz->txrx, temp_data32)) {
        ...
        furi_string_set_str(
            temp_str,
            subghz_wardriving_txrx_get_preset_name(subghz->txrx, furi_string_get_cstr(temp_str)));
        ...
        subghz_wardriving_txrx_set_tx_power(preset_data, preset_data_size, subghz->tx_power);
        ...
        FlipperFormat* fff_data = subghz_wardriving_txrx_get_fff_data(subghz->txrx);
        if(!strcmp(furi_string_get_cstr(temp_str), "RAW")) {
            //if RAW
            subghz->load_type_file = SubGhzLoadTypeFileRaw;
            subghz_protocol_raw_gen_fff_data(
                fff_data, file_path, subghz_wardriving_txrx_radio_device_get_name(subghz->txrx));
        } else {
        ...
        if(subghz_wardriving_txrx_load_decoder_by_name_protocol(
            ...
            SubGhzProtocolStatus status = subghz_protocol_decoder_base_deserialize(
                subghz_wardriving_txrx_get_decoder(subghz->txrx), fff_data);
...
void subghz_save_to_file(void* context) {
    ...
    if(subghz_path_is_file(subghz->file_path)) {
        subghz_save_protocol_to_file(
            subghz,
            subghz_wardriving_txrx_get_fff_data(subghz->txrx),
